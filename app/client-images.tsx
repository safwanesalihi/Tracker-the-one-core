...
export default function ClientImages({ client, workspaceId, canEdit, onChange, onError }: Props) {
  ...
  async function pick(kind: ImageKind, file: File | undefined) {
    ...
    try {
      ...
      await onChange({ [kind]: id }, kind === 'logo' ? 'Logo mis à jour' : 'Bannière mise à jour');
  ...
  async function remove(kind: ImageKind) {
    if (busy) return;
    setBusy(kind);
    try { await onChange({ [kind]: '' }, kind === 'logo' ? 'Logo retiré' : 'Bannière retirée'); }
  ...
  return <div className={`client-hero ${client.banner ? 'has-banner' : ''}`}>
    <div className="client-banner">
      ...
      {canEdit && <div className="client-image-tools">
        ...
        <button type="button" className="btn" disabled={!!busy} onClick={() => bannerInput.current?.click()}>{busy === 'banner' ? <Loader2 size={14} className="spin" /> : <ImagePlus size={14} />}{client.banner ? 'Changer la bannière' : 'Ajouter une bannière'}</button>
        {client.banner && <button type="button" className="icon-button" aria-label="Retirer la bannière" disabled={!!busy} onClick={() => void remove('banner')}><Trash2 size={14} /></button>}
      </div>}
    </div>
    <div className="client-hero-mark">
      ...
      {canEdit && <div className="client-logo-tools">
        ...
        <button type="button" className="text-link" disabled={!!busy} onClick={() => logoInput.current?.click()}>{busy === 'logo' ? <Loader2 size={13} className="spin" /> : <ImagePlus size={13} />}{client.logo ? 'Changer le logo' : 'Ajouter un logo'}</button>
        {client.logo && <button type="button" className="text-link" disabled={!!busy} onClick={() => void remove('logo')}><Trash2 size={13} />Retirer</button>}
      </div>}
    </div>
  </div>;
}
